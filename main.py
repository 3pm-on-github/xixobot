...
class XixoBot(discord.Client):
    ...
    async def on_message(self, message):
        ...
        guild = self.get_guild(1409280301666013286)
        await huh(message, guild)
        if random.randint(1, 1000) == 1:
            await message.channel.send("<@" + str(random.choice(self.useridlist)) + ">", file=discord.File('choccy.png'))
        ...
        if re.search(r"(ev+i+l)", message.content, re.IGNORECASE) and not self.evilmode:
            ...
            await message.channel.send("evil mode is currently under maintenance. please check later!")
